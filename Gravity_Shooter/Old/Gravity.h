...
struct Ship {
    ...
    Ship(double x=0,double y=0,int color=0)
        : x(x), y(y), color(color), isFailed(false), shootCounts(0) {}
};
...
struct Bomb {
    ...
    Bomb(double x=0,double y=0,double vx=0,double vy=0,int id=-1,int color=0)
        : x(x), y(y), vx(vx), vy(vy), id(id), color(color), isCrashed(false) {}
};
...
struct Planet {
	...
	int color;//颜色（6位16进制颜色代码）
//	bool isDeleted;
    Planet(double x=0,double y=0,double r=0,int color=0)
        : x(x), y(y), r(r), M(r*r), color(color) {}
};
...
//			this->minR=minR;
    ...
    void addPlannet(double x,double y,double r) {
...
public:
	...
	int calculate()
	{
        ...
        int plannetsCnt=shooter.getPlanetsNum();
		double ax=0,ay=0;
